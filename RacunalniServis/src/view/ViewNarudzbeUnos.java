...
/**
 * Prozor za unos nove narudžbe. Narudžba se veže za kupca pa je
 * njegov izbor realiziran kroz combobox.
 * @author nives
 *
 */
public class ViewNarudzbeUnos extends JDialog {
    ...
    public ViewNarudzbeUnos() {
        this(null, "", false);
    }

    public ViewNarudzbeUnos(Frame parent, String title, boolean modal) {
     ...
     * @param e
     */
    private void spremanjeNarudzbe(ActionEvent e) {
    	DateFormat dateFormat = new SimpleDateFormat("dd.MM.yyyy");
    	
    	Narudzba narudzba = new Narudzba();
    	Kupac kupac = (Kupac) jComboBox1.getSelectedItem();
    	narudzba.setIdKupca(kupac.getIdKupca());
        try {
			narudzba.setDatum(new Date(dateFormat.parse(jTextField2.getText()).getTime()));
		} catch (ParseException e1) {
			JOptionPane.showMessageDialog(null, "Neispravan format datuma (dd.mm.gggg).");
			return;
		}
    	controllerNarudzbe.spremiNarudzbu(narudzba);
        setVisible(false);
    }
}
